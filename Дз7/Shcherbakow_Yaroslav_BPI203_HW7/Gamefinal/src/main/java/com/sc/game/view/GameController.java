...
import static com.sc.game.repository.Constants.DATETIME_PATTERN;
...
public class GameController implements ActionListener, TimerListener {
    ...
    @FXML
    private Button getTopResultsButton;

    @FXML
    private Button getTopResultsOnLoginButton;
    ...
    @FXML
    protected void onTopResultsButtonClick() {
        this.client.getTopResults();
    }
    ...
    public void onLostConnection() {
        ...
        Platform.runLater(() -> {
            if (Objects.equals(stateLabel.getText(), "Connect canceled. Max number of players reached")) {
                return;
            }
    ...
    public void onGetName() {
        Platform.runLater(() ->
        {
            ...
            getTopResultsOnLoginButton.setVisible(true);
        });
    }
    ...
    @Override
    public void onDisplayTopResults(Vector<TopResultsModel> topResultsModel) {
        Platform.runLater(() ->
        {
            var stringBuilder = new StringBuilder();
            var zoneTimeOffset = ZonedDateTime.now().getOffset().getTotalSeconds();
            for (var result : topResultsModel) {
                stringBuilder.append(result.getFinishGameTime().plusSeconds(zoneTimeOffset).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))).append(" ")
                        .append(result.getLogin()).append(" with step count: ")
                        .append(result.getStepNumber()).append(" in ").append(result.getStepTime()).append("ms\n");
            }

            var dialog = new Alert(Alert.AlertType.INFORMATION);
            dialog.setResizable(true);
            dialog.getDialogPane().setPrefSize(500, 320);
            dialog.setTitle("Top 10");
            dialog.setHeaderText("Top 10 results:");
    ...
    private void hideAllControls() {
        ...
        getTopResultsButton.setVisible(false);
        getTopResultsOnLoginButton.setVisible(false);
    }
    ...
    private void showResultControls() {
        ...
        getTopResultsButton.setVisible(true);
    }
